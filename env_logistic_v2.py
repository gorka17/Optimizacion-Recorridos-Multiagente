...
class TruckTrajectoryMAEnv(BaseMAEnv):
    def __init__(self, num_agents: int = 2, cities_left = [], render=False):
        ...
        self.all_cities = {'Madrid': 0, 'Barcelona': 1, 'Valencia': 2, 'Sevilla': 3,
                           ...
                           'Badajoz': 9, 'Zaragoza': 10, 'C. Real': 11}

        # Asignamos una acción a cada movimiento posible en cada ciudad.
        self.cities = {
            'A Coruna': {'Leon': 0, 'Salamanca': 1, 'Bilbao': 2, 'Badajoz': 3},
            'Leon': {'A Coruna': 0, 'Salamanca': 1, 'Bilbao': 2, 'Madrid': 3, 'Zaragoza': 4},
            'Salamanca': {'A Coruna': 0, 'Leon': 1, 'Madrid': 2, 'Badajoz': 3, 'C. Real': 4, 'Sevilla': 5},
            'Sevilla': {'Salamanca': 0, 'Badajoz': 1, 'Murcia': 2, 'C. Real': 3},
            'Murcia': {'Sevilla': 0, 'Madrid': 1, 'Valencia': 2, 'C. Real': 3},
            'Valencia': {'Murcia': 0, 'Madrid': 1, 'Barcelona': 2, 'C. Real': 3, 'Zaragoza': 4},
            'Barcelona': {'Valencia': 0, 'Zaragoza': 1, 'Bilbao': 2},
            'Bilbao': {'Barcelona': 0, 'Madrid': 1, 'Leon': 2, 'A Coruna': 3, 'Zaragoza': 4},
            'Madrid': {'Badajoz': 0, 'Salamanca': 1, 'Leon': 2, 'Bilbao': 3, 'Zaragoza': 4, 'Valencia': 5, 'Murcia': 6, 'C. Real': 7},
            'Badajoz': {'A Coruna': 0, 'Salamanca': 1, 'Madrid': 3, 'C. Real': 4, 'Sevilla': 5},
            'Zaragoza': {'Madrid': 0, 'Leon': 1, 'Bilbao': 2, 'Barcelona': 3, 'Valencia': 4},
            'C. Real': {'Madrid': 0, 'Valencia': 1, 'Murcia': 2, 'Sevilla': 3, 'Badajoz': 4, 'Salamanca': 5}
        }

        # Asignamos las distancias a cada trayecto.
        self.distances = {
        'A Coruna': {'Leon': 220, 'Salamanca': 452, 'Bilbao': 550, 'Badajoz': 684},
        'Leon': {'A Coruna': 220, 'Salamanca': 205, 'Bilbao': 230, 'Madrid': 335, 'Zaragoza': 475},
        'Salamanca': {'A Coruna': 452, 'Leon': 205, 'Madrid': 212, 'Badajoz': 293, 'C. Real': 354, 'Sevilla': 400},
        'Sevilla': {'Salamanca': 400, 'Badajoz': 212, 'Murcia': 585, 'C. Real': 328},
        'Murcia': {'Sevilla': 585, 'Madrid': 400, 'Valencia': 240, 'C. Real': 363},
        'Valencia': {'Murcia': 240, 'Madrid': 352, 'Barcelona': 349, 'C. Real': 354, 'Zaragoza': 314},
        'Barcelona': {'Valencia': 349, 'Zaragoza': 308, 'Bilbao': 610},
        'Bilbao': {'Barcelona': 610, 'Madrid': 400, 'Leon': 230, 'A Coruna': 550, 'Zaragoza': 303},
        'Madrid': {'Badajoz': 400, 'Salamanca': 212, 'Leon': 335, 'Bilbao': 400, 'Zaragoza': 306, 'Valencia': 352, 'Murcia': 400, 'C. Real': 190},
        'Badajoz': {'A Coruna': 684, 'Salamanca': 293, 'Madrid': 400, 'C. Real': 306, 'Sevilla': 212},
        'Zaragoza': {'Madrid': 306, 'Leon': 475, 'Bilbao': 303, 'Barcelona': 308, 'Valencia': 314},
        'C. Real': {'Madrid': 190, 'Valencia': 354, 'Murcia': 363, 'Sevilla': 328, 'Badajoz': 306, 'Salamanca': 354}
        }
        ...
        for i in range(num_agents):
            ...
            self.register_agent(
                agent_id=agent_name,
                observation_space=MultiDiscrete([12] * self.num_agents + [2] * 12),
    ...
    def reset(self, seed=None):
        ...
        # Asignar ciudades random para agente al inicio.
    ...
    def get_env_state_results(self):
        ...
        for agent_id in self.agents:
            ...
            # Si no se escoge acción válida se penaliza, si se explora hay reward y si no también se penaliza.
            if self.penalty:
                reward -= 10.0

            if self.previous_cities[agent_id] != current_city:
                # Se tienen en cuenta los km recorridos.
                reward -= self.distances[self.previous_cities[agent_id]][current_city] / 50.0
            ...
            # Si los agentes van al mismo lugar se penaliza.
        ...
        # Premio si todos visitan ciudades nuevas y diferentes entre sí.
        if all(self.extra_reward.values()):
            self.streak += 1
            for agent_id in self.agents:
                rewards[agent_id] += 10.0 * (1 + 0.3) ** self.streak
        else:
            self.streak =- 1
        ...
        if (terminated or truncated) and self.render:
            self.draw_trajectories(img_path='Mapa_nuevo.jpg')
